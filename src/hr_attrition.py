...
# Finally, we will test the impact of variations in some of the most important features, as indicated by SHAP, on the predicted probability of turnover. We will test cases of percentage increases or decreases in all variables, by providing an initial analysis on 300 samples, and then plotting the impact of a 10% and 50% increase and decrease on the 5 most impactful variables of our model.

# We will have in consideration the SHAP of the XGBoost model, as it was the best performing one.

# Get top 5 features by mean absolute SHAP values from the TreeExplainer analysis

# We start by calculating the mean absolute SHAP values for each feature
mean_abs_shap = np.abs(shap_values).mean(axis=0)

feature_importance_whatif = pd.DataFrame({
    'feature': features_moderate,
    'mean_abs_shap': mean_abs_shap
}).sort_values('mean_abs_shap', ascending=False)

print("Top 5 Most Important Features by TreeExplainer SHAP:")
print(feature_importance_whatif.head(5))

# Get top 5 features
top_5_features = feature_importance_whatif.head(5)['feature'].tolist()
print(f"\nTop 5 features for what-if analysis: {top_5_features}")

# %%
# What-If Scenario Analysis: Test +10% and -10% changes for top 5 SHAP features

def apply_whatif_scenario(df, feature_name, change_pct):
    """
    Apply a percentage change to a feature in the dataframe.
    
    Parameters:
    - df: input dataframe (must be a DataFrame with columns)
    - feature_name: name of feature to modify
    - change_pct: percentage change (e.g., 0.10 for +10%, -0.10 for -10%)
    
    Returns:
    - modified dataframe with the feature changed
    """
    df_modified = df.copy()
    
    # Only apply change to numeric features that exist
    if feature_name in df_modified.columns:
        df_modified[feature_name] = df_modified[feature_name] * (1 + change_pct)
    
    return df_modified


# Convert X_shap to DataFrame for easier manipulation (if it's numpy array)
if isinstance(X_shap, np.ndarray):
    X_shap_df = pd.DataFrame(X_shap, columns=features_moderate)
else:
    X_shap_df = X_shap.copy()

# Store results for what-if analysis
whatif_results = []

print(f"\nProcessing what-if scenarios for {len(X_shap_df)} samples...")

for idx in range(len(X_shap_df)):
    # Original prediction using the XGBoost model
    original_sample = X_shap_df.iloc[[idx]]
    ...
    whatif_results.append({
        ...
        'change_pct': 0,
        'predicted_proba': original_proba,
        'prediction': 1 if original_proba >= 0.5 else 0
    })
    
    # Test +10% and -10% for each top 5 feature
    for feature in top_5_features:
        for change in [0.10, -0.10]:
            modified_sample = apply_whatif_scenario(original_sample, feature, change)
            
            # Get predictions for modified sample
            modified_proba = model.predict_proba(modified_sample)[0, 1]
            
            whatif_results.append({
                'sample_idx': idx,
                'scenario': f'{feature}_{"+10%" if change > 0 else "-10%"}',
                'feature_changed': feature,
                'change_pct': change,
                'predicted_proba': modified_proba,
                'prediction': 1 if modified_proba >= 0.5 else 0
            })

whatif_df = pd.DataFrame(whatif_results)

print(f"What-If Scenario Analysis completed for {len(X_shap_df)} samples")
print(f"Total scenarios tested: {len(whatif_df)} rows")
print("\nFirst few rows of what-if analysis:")
print(whatif_df.head(12))

# %%
# Summary statistics provided by this analysis: Average impact of +10% and -10% changes

summary_stats = []

for feature in top_5_features:
    # Get original baseline
    baseline = whatif_df[whatif_df['scenario'] == 'original']['predicted_proba'].mean()
    
    # Get +10% change impact
    plus_10 = whatif_df[whatif_df['scenario'] == f'{feature}_+10%']['predicted_proba'].mean()
    impact_plus = plus_10 - baseline
    
    # Get -10% change impact
    minus_10 = whatif_df[whatif_df['scenario'] == f'{feature}_-10%']['predicted_proba'].mean()
    impact_minus = minus_10 - baseline
    ...
    summary_stats.append({
        ...
        'proba_+10%': plus_10,
        'proba_-10%': minus_10,
        'impact_+10%': impact_plus,
        'impact_-10%': impact_minus,
        'abs_impact_avg': (abs(impact_plus) + abs(impact_minus)) / 2
    })

summary_df = pd.DataFrame(summary_stats)
print("\n=== What-If Analysis Summary ===")
print("Impact of ±10% changes on XGBoost predicted attrition probability:")
print(summary_df.to_string(index=False))

# %%
# Visualization: Impact of feature changes on attrition probability

plt.figure(figsize=(12, 6))


features_list = summary_df['feature'].tolist()
impacts_plus = summary_df['impact_+10%'].tolist()
impacts_minus = summary_df['impact_-10%'].tolist()

x = np.arange(len(features_list))
width = 0.35

# Bars creation with custom colors
bars1 = plt.bar(x - width/2, impacts_plus, width, label='+10% increase', 
                alpha=0.9, color='#1e3a5f')  # Azul escuro
bars2 = plt.bar(x + width/2, impacts_minus, width, label='-10% decrease', 
                alpha=0.9, color='#6b9bc3')  # Azul claro

# Labelling
for bar in bars1:
    height = bar.get_height()
    plt.text(bar.get_x() + bar.get_width()/2., height,
             f'{height:+.2%}',  # Formato: +5.23%
...
for bar in bars2:
    height = bar.get_height()
    plt.text(bar.get_x() + bar.get_width()/2., height,
             f'{height:+.2%}',  # Formato: -3.45%
             ha='center', va='bottom' if height >= 0 else 'top', 
             fontsize=9, fontweight='bold')

plt.xlabel('Feature', fontsize=11)
plt.ylabel('Impact on Attrition Probability', fontsize=11)
plt.title('What-If Analysis: Impact of ±10% Feature Changes', fontsize=13, fontweight='bold')
plt.xticks(x, features_list, rotation=45, ha='right', fontsize=10)
...
# %%
# What-If Scenario Analysis 2: Testing +50% and -50% changes for top 5 SHAP features

# Now, we want to try if the % of variation is larger, e.g., ±50%. We want to validate if the impact increases substantially or not. In theory, a 50% increase or decrease is not something that a company will be able to, immediately, do, but it is interesting to see how the model reacts to such changes, and how much impact will it have on attrition probability.

# Store results for ±50% scenarios
whatif_results_50 = []

print(f"\nProcessing what-if scenarios (±50%) for {len(X_shap_df)} samples...")
...
for idx in range(len(X_shap_df)):
    ...
    # Test +50% and -50% for each top 5 feature
    for feature in top_5_features:
        for change in [0.50, -0.50]:
            modified_sample = apply_whatif_scenario(original_sample, feature, change)
            modified_proba = model.predict_proba(modified_sample)[0, 1]
            
            whatif_results_50.append({
                'sample_idx': idx,
                'scenario': f'{feature}_{"+50%" if change > 0 else "-50%"}',
                'feature_changed': feature,
                'change_pct': change,
                'predicted_proba': modified_proba,
                'prediction': 1 if modified_proba >= 0.5 else 0
...
whatif_df_50 = pd.DataFrame(whatif_results_50)

# Summary statistics for ±50%
summary_stats_50 = []

for feature in top_5_features:
    baseline = whatif_df_50[whatif_df_50['scenario'] == 'original']['predicted_proba'].mean()
    
    plus_50 = whatif_df_50[whatif_df_50['scenario'] == f'{feature}_+50%']['predicted_proba'].mean()
    impact_plus = plus_50 - baseline
    
    minus_50 = whatif_df_50[whatif_df_50['scenario'] == f'{feature}_-50%']['predicted_proba'].mean()
    impact_minus = minus_50 - baseline
    
    summary_stats_50.append({
        'feature': feature,
        'baseline_proba': baseline,
        'proba_+50%': plus_50,
        'proba_-50%': minus_50,
        'impact_+50%': impact_plus,
        'impact_-50%': impact_minus
    })

summary_df_50 = pd.DataFrame(summary_stats_50)

print("\n=== What-If Analysis Summary (±50%) ===")
print(summary_df_50.to_string(index=False))

# %%
# Visualization: ±50% impact
plt.figure(figsize=(12, 6))

features_list = summary_df_50['feature'].tolist()
impacts_plus = summary_df_50['impact_+50%'].tolist()
impacts_minus = summary_df_50['impact_-50%'].tolist()

x = np.arange(len(features_list))
width = 0.35

bars1 = plt.bar(x - width/2, impacts_plus, width, label='+50% increase', 
                alpha=0.9, color='#1e3a5f')
bars2 = plt.bar(x + width/2, impacts_minus, width, label='-50% decrease', 
                alpha=0.9, color='#6b9bc3')
...
plt.xlabel('Feature', fontsize=11)
plt.ylabel('Impact on Attrition Probability', fontsize=11)
plt.title('What-If Analysis: Impact of ±50% Feature Changes', fontsize=13, fontweight='bold')
plt.xticks(x, features_list, rotation=45, ha='right', fontsize=10)
plt.legend(fontsize=10)
plt.axhline(y=0, color='black', linestyle='-', linewidth=0.8)
plt.grid(axis='y', alpha=0.3)
...
# We can conclude that OverTime is clearly the variable which might have a larger impact on attrition probability, with both 10% and 50% decreases leading to a decrease of 10% in the number of possible departures.

# MonthlyIncome seems to be the variable which suffers the most impact from an increase from ±10% to ±50%. Nevertheless, it is probably unbearable for a company to increase or decrease salaries by 50% in a short period of time., meaning this exercise is merely theoretical. This assumptions should be analyzed with caution, since randomness in the samples might have an impact on the results.

# Interestingly, a reduction of 10% in Age seems to have a positive impact on attrition probability, which is counter-intuitive, considering the results from the SHAP analysis. Engagement Index also indicates a positive effect in both its increase and decrease. This might be a result of interactions with other variables in the dataset, which should be further analyzed.

# With this analysis we conclude our work on the project, having built a robust model for predicting employee attrition, and having provided insights on the main drivers of this phenomenon within the company. Nevertheless, further research should be conducted to validate these findings, and to explore potential interactions between all variables, as well as exploring new ones, such as absenteeism rates, working model (presential, hybrid, remote), and others.